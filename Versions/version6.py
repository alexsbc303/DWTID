...
class PerformanceMonitor:
    ...
    def optimize_memory() -> dict:
        ...
        gc.collect()
        return PerformanceMonitor.get_memory_usage()
    
    @staticmethod
    def memory_efficient_read_csv(filepath: str, chunk_size: int = 50000) -> Iterator[pd.DataFrame]:
        """Read CSV in chunks to reduce memory usage."""
        try:
            for chunk in pd.read_csv(filepath, 
                                   encoding='latin-1', 
                                   on_bad_lines='skip', 
                                   engine='c',  # Much faster than 'python'
    ...
    def show_memory_stats():
        """Display current memory usage in sidebar - disabled for production."""
...
def load_datasets_optimized():
    ...
    # Check if we have enough memory for large datasets
    use_chunked_loading = memory_usage['available_mb'] < 2000
    ...
    try:
        if use_chunked_loading:
            ...
            for chunk in PerformanceMonitor.memory_efficient_read_csv('Clearnedup_ALL_7.csv', 
                ...
                if memory['rss_mb'] > 3000:  # More than 3GB used
                    break
...
def compute_sentiment_efficient(text_series, sample_size=None):
    ...
    try:
        # Memory-efficient unique text processing
        if show_progress:
        ...
        # Adaptive sampling based on dataset size and memory
        memory_usage = PerformanceMonitor.get_memory_usage()
        if memory_usage['rss_mb'] > 2000:  # High memory usage
            effective_sample_size = min(sample_size // 2, len(unique_texts))
        else:
        ...
        # Vectorized sentiment computation with batching
        sentiment_map = {}
        batch_size = 1000  # Process in batches to control memory
        processed = 0
        
        for i in range(0, len(sampled_texts), batch_size):
            batch = sampled_texts.iloc[i:i+batch_size]
            ...
            for text in batch:
                ...
                if len(text_str) > 0:
                    try:
                        ...
                        sentiment_map[text] = 0.0  # Default for problematic texts
                else:
                    sentiment_map[text] = 0.0
            
            processed += len(batch)
            if show_progress and len(sampled_texts) > 0:
                progress = 0.2 + (processed / len(sampled_texts)) * 0.7
        ...
        # Efficient mapping back to original series
        result = text_series.map(sentiment_map)
        
        # Fill unmapped values with neutral sentiment
        unmapped_count = result.isna().sum()
        if unmapped_count > 0:
...
def main():
    ...
    if analysis_choice == 'Threat Dashboard':
        ...
        # Then show LDA if available with better UI
        if GENSIM_AVAILABLE:
            st.markdown("---")
            show_enhanced_topic_modeling(crawled_result_df, text_column='combined_text', title="Advanced Topic Modeling (LDA)")
        ...
        if GENSIM_AVAILABLE:
            st.markdown("---")
            show_enhanced_topic_modeling(preprocessed_forum_df, text_column='Post Content', title="Advanced Topic Modeling (LDA)")
...
def show_enhanced_topic_modeling(df, text_column='Post Content', title="Advanced Topic Modeling (LDA)"):
    ...
    with col1:
        num_topics = st.slider("📊 Number of Topics", min_value=3, max_value=15, value=8, 
                              help="Select how many topics to extract from the text")
    with col2:
        num_words = st.slider("🔤 Words per Topic", min_value=5, max_value=20, value=10,
                             help="Number of representative words to show for each topic")
    with col3:
        sample_size = st.selectbox("📄 Sample Size", 
                                  options=[1000, 5000, 10000, 25000, "All"],
                                  index=2,
                                  help="Number of documents to analyze (larger = slower but more comprehensive)")
    
    # Advanced options in expander
    with st.expander("🔧 Advanced Settings"):
        col1, col2 = st.columns(2)
        with col1:
            min_word_freq = st.slider("Minimum Word Frequency", min_value=1, max_value=10, value=2,
                                    help="Ignore words that appear fewer times than this")
            passes = st.slider("Training Passes", min_value=1, max_value=20, value=10,
                             help="Number of passes through the corpus during training")
        with col2:
            max_word_freq = st.slider("Maximum Word Frequency (%)", min_value=10, max_value=95, value=80,
                                    help="Ignore words that appear in more than this % of documents")
            iterations = st.slider("Iterations per Pass", min_value=10, max_value=100, value=50,
                                 help="Number of iterations in each training pass")
    
    if st.button("🚀 Run Advanced Topic Analysis", type="primary"):
        with st.spinner("Running advanced LDA analysis... This may take a while."):
            try:
                ...
                status_text.text("Building vocabulary...")
                ...
                # Filter extremes
                dictionary.filter_extremes(no_below=min_word_freq, no_above=max_word_freq/100)
                ...
                # Train LDA model with user parameters
                lda_model = LdaModel(
                    corpus=corpus,
                    id2word=dictionary,
                ...
                # Results display
                st.success(f"✅ Successfully identified {num_topics} topics from {len(text_data):,} documents!")
                ...
                try:
                    ...
                    st.metric("🎯 Topic Coherence Score", f"{coherence_lda:.3f}", 
                             help="Higher scores indicate better topic quality (0-1 scale)")
                except:
                    pass
                ...
                for i, tab in enumerate(topic_tabs):
                    with tab:
                        ...
                        try:
                            ...
                            ax.imshow(wordcloud, interpolation='bilinear')
                            ax.axis('off')
                            ax.set_title(f'Topic {i+1} Word Cloud', fontsize=16, fontweight='bold')
                ...
                st.info("The system will fall back to keyword analysis for this dataset.")

def show_topic_modeling(df, text_column='Post Content', title="Topic Modeling", num_topics=5, num_words=10):
    """Perform topic modeling on specified text column with better error handling."""
    st.header(f"🔬 {title}")
    ...
    with st.spinner("Running LDA Topic Modeling... This may take a while."):
        try:
            ...
            # Enhanced stop words for dark web analysis
            stop_words.update(['said', 'say', 'get', 'go', 'know', 'make', 'see', 'come', 'could', 'time', 'would', 'also', 'back', 'them', 'well', 'way', 'much', 'take', 'than', 'only', 'think', 'now', 'people', 'may', 'come', 'its', 'over', 'some', 'what', 'out', 'so', 'up', 'year', 'when', 'them', 'my', 'me', 'will', 'if', 'no', 'do', 'would', 'who', 'so', 'about', 'out', 'many', 'then', 'them', 'me', 'well', 'some'])
            ...
            def preprocess_text(text):
                try:
                    ...
                    try:
                        ...
                        # Final fallback if NLTK tokenizer fails
                        import re
                        tokens = re.findall(r'\b\w+\b', str(text).lower())
                return [word for word in tokens if word.isalpha() and len(word) > 2 and word not in stop_words]

            # Use combined_text if available, otherwise use specified column
            ...
            # Sample data for performance and filter non-empty texts
            ...
            for text in text_sample:
                ...
                if len(doc) > 3:  # Only include documents with more than 3 words
                    processed_docs.append(doc)
            
            if len(processed_docs) < 10:
                st.warning("Not enough meaningful text data for topic modeling. Showing keyword analysis instead.")
            ...
            if len(dictionary) < 10:
                st.warning("Insufficient vocabulary for topic modeling. Showing keyword analysis instead.")
                show_meaningful_keyword_analysis(df, title.replace("Topics", "Keywords"))
                return
            
            corpus = [dictionary.doc2bow(doc) for doc in processed_docs]
            
            # Train LDA model with better parameters
            lda_model = LdaModel(
                corpus=corpus,
                id2word=dictionary,
                num_topics=num_topics,
            ...
            with col1:
                for i, topic in enumerate(topics):
                    st.markdown(f"**Topic {topic[0]}:**")
                    # Clean up topic display
                    topic_words = topic[1].replace('"', '').replace('*', '').replace('+', ', ')
                    st.markdown(f"*{topic_words}*")
                    st.markdown("---")
            
            with col2:
                st.subheader("Analysis Summary")
                st.metric("Documents Analyzed", f"{len(processed_docs):,}")
                st.metric("Vocabulary Size", f"{len(dictionary):,} unique terms")
                st.metric("Topics Discovered", num_topics)
                
                # Show topic coherence if possible
                try:
                    coherence = lda_model.log_perplexity(corpus)
                    st.metric("Model Perplexity", f"{coherence:.2f}")
                except:
                    pass
                    
        except Exception as e:
            st.error(f"Topic modeling failed: {e}")
            st.info("Falling back to keyword analysis...")
            show_meaningful_keyword_analysis(df, title.replace("Topics", "Keywords"))
